...
DEFINE_ENUM_STRINGS(DEVICE_TWIN_EVENT_TYPE, DEVICE_TWIN_EVENT_TYPE_VALUES)
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (json_object_set_string(info_json_obj, "status", ENUM_TO_STRING(IOTHUB_CLIENT_CONNECTION_STATUS, info->status)) != JSONSuccess)
                {
                    LogError("Failed serializing connection status");
                    json_value_free(info_json);
                }
                else if (json_object_set_string(info_json_obj, "reason", ENUM_TO_STRING(IOTHUB_CLIENT_CONNECTION_STATUS_REASON, info->reason)) != JSONSuccess)
                {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (json_object_dotset_string(info_json_obj, "send.result", ENUM_TO_STRING(IOTHUB_CLIENT_CONFIRMATION_RESULT, info->send_callback_result)) != JSONSuccess)
                {
...
int iothub_client_statistics_add_telemetry_info(IOTHUB_CLIENT_STATISTICS_HANDLE handle, TELEMETRY_EVENT_TYPE type, TELEMETRY_INFO* info)
{
    ...
    else
    {
        ...
        if (list_item == NULL)
        {
            ...
            {
                LogError("Telemetry info not found for message %zu (%d)", info->message_id, (int)type);
                result = __FAILURE__;
            }
            ...
            {
                LogError("Failed adding telemetry info (message id: %zu)", info->message_id);
                free(queued_info);
                result = __FAILURE__;
            }
            else
        ...
        else
        {
            ...
            {
                LogError("Failed retrieving queued telemetry info (message id: %zu)", info->message_id);
                result = __FAILURE__;
            }
            else
            {
...
int iothub_client_statistics_add_c2d_info(IOTHUB_CLIENT_STATISTICS_HANDLE handle, C2D_EVENT_TYPE type, C2D_MESSAGE_INFO* info)
{
    ...
    else
    {
        ...
        if (list_item == NULL)
        {
            if (type != C2D_QUEUED)
            {
                LogError("C2D message info not found for message %lu (%zu)", info->message_id, (int)type);
                result = __FAILURE__;
            }
            ...
            {
                LogError("Failed adding c2d message info (message id: %zu)", info->message_id);
                free(queued_info);
                result = __FAILURE__;
            }
            else
        ...
        else
        {
            ...
            {
                LogError("Failed retrieving queued c2d message info (message id: %zu)", info->message_id);
                result = __FAILURE__;
            }
            else
            {
                ...
                {
                    LogError("C2D message %zu in queue, invalid event type (%d)", info->message_id, (int)type);
                    result = __FAILURE__;
                }
            }
        }
...
int iothub_client_statistics_add_device_method_info(IOTHUB_CLIENT_STATISTICS_HANDLE handle, DEVICE_METHOD_EVENT_TYPE type, DEVICE_METHOD_INFO* info)
{
    ...
    else
    {
        ...
        if (list_item == NULL)
        {
            ...
            {
                LogError("Failed adding device methods info (method id: %zu)", info->method_id);
                free(queued_info);
                queued_info = NULL;
            }
            else
        ...
        else
        {
            ...
            {
                LogError("Failed retrieving queued device method info (method id: %zu)", info->method_id);
            }
        }
        ...
        if (queued_info != NULL)
        {
            ...
            {
                LogError("Device method %zu in queue; invalid event type (%d)", info->method_id, (int)type);
            }
        }
    }
...
int iothub_client_statistics_add_device_twin_desired_info(IOTHUB_CLIENT_STATISTICS_HANDLE handle, DEVICE_TWIN_EVENT_TYPE type, DEVICE_TWIN_DESIRED_INFO* info)
{
    ...
    else
    {
        ...
        LogInfo("type=%s, id=%zu)", ENUM_TO_STRING(DEVICE_TWIN_EVENT_TYPE, type), info->update_id);

        if (list_item == NULL)
        {
            if (type != DEVICE_TWIN_UPDATE_SENT)
            {
                LogError("Invalid info type (update id=%zu, type=%d)", info->update_id, (int)type);
                result = __FAILURE__;
            }
            ...
            {
                LogError("Failed adding device twin info (update id: %zu)", info->update_id);
                free(queued_info);
                result = __FAILURE__;
            }
            else
        ...
        else
        {
            ...
            {
                LogError("Failed retrieving queued device twin info (update id: %zu)", info->update_id);
                result = __FAILURE__;
            }
            ...
            {
                LogError("Device twin %zu in queue; invalid event type (%d)", info->update_id, (int)type);
                result = __FAILURE__;
            }
        }
    }
